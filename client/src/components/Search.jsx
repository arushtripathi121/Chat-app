...
const Search = ({setUserData}) => {
    ...
    return (
        <div className=''>
            <div className='h-[15svh] bg-white border-b border-gray-300'>
                ...
                <p className='px-5 flex flex-row items-center bg-gray-100'>
                    <input
                        ...
                        className='w-full outline-none px-3 py-2 text-lg font-medium rounded-lg bg-gray-100'
                        placeholder='Search or start a new chat'
                    />
                    <IoMdClose className='w-5 h-5' onClick={closeResults} />
                </p>
                    ...
                    {searchResults.map(m => (
                        <div key={m._id} className='flex flex-row items-center justify-between w-full px-2 py-2 border-b border-black cursor-pointer hover:bg-slate-400' onClick={() => setUserData(m)}>
                            ...
                            <div className='flex flex-col items-center'>
                                ...
                                <p className='text-xl'>{m.userName}</p>
                            </div>
                        </div>
                    ))}
                </div>
